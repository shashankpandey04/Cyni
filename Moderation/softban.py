...
class SoftBan(commands.Cog):
    ...
    async def softban(self, ctx, user: discord.Member, *, reason: str):
        ...
        try:
            ...
            await ctx.send(f"{caution} User {user} have been softbanned for {reason}.")
            await user.ban(reason=reason)
            await user.unban(reason=reason)
            embed = discord.Embed(title="User Softbanned", color=0x2F3136)
            ...
            #send it to user too.
            embed = discord.Embed(title=f"You got Softbanned from {ctx.guild} for {reason}", color=0x2F3136)
            embed.add_field(name=f"{arrow} Moderator", value=ctx.author.mention, inline=False)
            embed.add_field(name=f"{arrow} Reason", value=reason, inline=False)
